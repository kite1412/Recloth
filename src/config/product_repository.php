...
function recloth_base_product_select(array $schema): string
{
    ...
    return "
        SELECT
            ...
            " . ($schema['has_image'] ? "IF(p.image LIKE 'uploads/%', CONCAT('../admin/', p.image), p.image)" : "''") . " AS image,
...
function recloth_fetch_product_images(PDO $pdo, int $productId, string $primaryImage = ''): array
{
    ...
    $stmt = $pdo->prepare("SELECT IF(image_url LIKE 'uploads/%', CONCAT('../admin/', image_url), image_url) FROM product_images WHERE product_id = :id ORDER BY sort_order ASC, id ASC");
